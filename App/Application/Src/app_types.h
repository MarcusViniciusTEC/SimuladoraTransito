...
#define NUMBER_OF_SIMULATION     15
...
typedef struct 
{
  uint8_t last_distance_axles_mts[NUMBER_OF_MAX_AXLES];
}axles_t;
...
typedef struct 
{
  ...
  uint8_t axles_end_distance_mts ;
  uint8_t axles_init_distance_mts;
}vehicle_t;

typedef struct 
{
  dually_weels_state_t  state;
  uint8_t  velocity_khm;
  vehicle_t vehicle [NUMBER_OF_TYPES_VEHICLE];
  uint8_t  gap [NUMBER_OF_TYPES_VEHICLE];
}traffic_update_t;
